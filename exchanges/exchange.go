...
func (e *Base) SetupDefaults(exch *config.ExchangeConfig) error {
	...
	e.CurrencyPairs.Pairs = make(map[assets.AssetType]*currency.PairStore)
	...
	for x := range assetTypes {
		...
		if p != nil {
			...
			log.Warnf("%s is nil", assetTypes[x])
		}
	}
